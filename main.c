...
processOut_t fCFS (processInfo_t process, int completionTime) {
    int turnAroundTime = completionTime - process.arrTime;
    int waitingTime = turnAroundTime - process.execTime;
    processOut_t processOut = {process.pid,turnAroundTime,waitingTime,0};
    return processOut;
}
...
void simulate(char filepath[], int algo) {
    ...
    processOut_t process_out = {0,0,0,0};
